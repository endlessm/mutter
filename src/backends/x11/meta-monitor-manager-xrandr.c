...
{
  MetaMonitorManager *manager = META_MONITOR_MANAGER (manager_xrandr);
  g_autoptr(GPtrArray) new_outputs = NULL;
  g_autoptr(GPtrArray) new_crtcs = NULL;
  gboolean needs_update = FALSE;
  GList *l;

  new_crtcs = g_ptr_array_new_with_free_func ((GDestroyNotify) meta_crtc_info_free);
  new_outputs = g_ptr_array_new_with_free_func ((GDestroyNotify) meta_output_info_free);

  for (l = meta_gpu_get_outputs (manager_xrandr->gpu); l != NULL; l = l->next)
    {
      MetaOutputInfo *output_info;
      MetaCrtcInfo *crtc_info;
      MetaOutput *output;
      MetaCrtc *crtc;
      const char *underscan_value;
      int current_width;
      int current_height;
      int target_width;
      int target_height;
      ...
      crtc_info->transform = crtc->transform;
      crtc_info->crtc = crtc;
      crtc_info->x = crtc->rect.x;
      crtc_info->y = crtc->rect.y;
      crtc_info->mode = crtc->current_mode;
      g_ptr_array_add (crtc_info->outputs, output);
      g_ptr_array_add (new_crtcs, crtc_info);

      underscan_value = meta_output_xrandr_get_underscan_value (output);

      /* Not a cropped output */
      if (g_strcmp0 (underscan_value, "crop") != 0)
        continue;

      current_width = crtc->current_mode->width;
      current_height = crtc->current_mode->height;
      ...
      if (current_width != target_width || current_height != target_height)
        {
          ...
          for (k = meta_gpu_get_modes (manager_xrandr->gpu); k != NULL; k = k->next)
            {
              MetaCrtcMode *mode = k->data;
  ...
  /* Handle under/overscan transitions */
  if (needs_update)
    {
      apply_crtc_assignments (manager,
                              FALSE,
...
{
  MetaMonitorManager *manager = META_MONITOR_MANAGER (manager_xrandr);
  MetaMonitorConfigManager *config_manager = NULL;
  MetaMonitorsConfig *config = NULL;
  MetaGpuXrandr *gpu_xrandr;
  XRRScreenResources *resources;
  gboolean is_hotplug;
  gboolean is_our_configuration;
  gboolean needs_update_for_underscan;
  ...
  if (is_hotplug && !needs_update_for_underscan)
    {
      meta_monitor_manager_on_hotplug (manager);
      return TRUE;
    }

  config_manager = meta_monitor_manager_get_config_manager (manager);

  if (needs_update_for_underscan)
    {
      config = meta_monitor_config_manager_create_fallback (config_manager);
      meta_monitor_config_manager_set_current (config_manager, config);
      meta_monitor_config_manager_save_current (config_manager);
    }
  else if (is_our_configuration)
    {
      config = meta_monitor_config_manager_get_current (config_manager);
    }
  else
